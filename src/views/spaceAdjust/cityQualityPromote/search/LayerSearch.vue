...
export default {
  ...
  methods: {
    ...
    // 图层字段列表
    async getChildFields() {
      const res = await getServerFields();
      if (res.code === 200) {
    ...
    async getTreeData() {
      ...
      if (res.code === 200) {
        ...
        if (this.treeData.length > 0 && this.treeData != null) {
          this.currentPid = this.treeData[0].pid;
          this.getSearchs();
          this.$nextTick(() => {
            this.$refs.treeData.setCurrentKey(this.currentPid);
          });
        }
      }
    },
